...
exports.getAllCom = (req, res) => {
    ...
    const sql = "SELECT comment.userId,comment.id, nom, prenom, comment.content, DATE_FORMAT(comment.date, '%d %b %Y %H:%i') AS date, comment.image FROM comment INNER JOIN post ON post.id = comment.postId LEFT JOIN user on user.id = comment.userId WHERE post.id=? ORDER BY comment.date DESC";
